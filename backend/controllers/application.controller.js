...
//ADMIN-> CAN SEE APPLICANTS OF PARTICULAR JOB
export const getApplicants = async (req, res) => {
    try {
        const jobId = req.params.id;
        const job = await Job.findById(jobId).populate({
            path: 'applications', options: { sort: { createdAt: -1 } }, populate: { path: 'applicant' }
        });
        //validation-job
        if (!job) {
...
export const applyJob = async (req, res) => {
    try {
        ...
        // is student already applied for this job
        const existingApplication = await Application.findOne({ job: jobId, applicant: userId });
        //validation-existing application
...
export const getAppliedJobs = async (req, res) => {
    try {
        ...
        console.log("Error while retriving student applied job : ", error);
    }
}
//STUDENT->SEE THEIR APPLICAION STATUS
